...
void ClassDemoApp::Setup()
{
	...
	topspinWav = Mix_LoadWAV("topspin.wav");
	backspinWav = Mix_LoadWAV("backspin.wav");
	nospinWav = Mix_LoadWAV("nospin.wav");
	lostballWav = Mix_LoadWAV("lostball.wav");
	themeOgg = Mix_LoadMUS("theme.ogg");
...
ClassDemoApp::~ClassDemoApp()
{
	...
	Mix_FreeChunk(topspinWav);
	Mix_FreeChunk(backspinWav);
	Mix_FreeChunk(nospinWav);
	Mix_FreeChunk(lostballWav);
	Mix_FreeMusic(themeOgg);
	SDL_Quit();
}
...
void ClassDemoApp::CollisionSound(float spin)
{
	if (spin > 0)
		Mix_PlayChannel(-1, topspinWav, 0);
	else if (spin < 0)
		Mix_PlayChannel(-1, backspinWav, 0);
	else
		Mix_PlayChannel(-1, nospinWav, 0);
}
